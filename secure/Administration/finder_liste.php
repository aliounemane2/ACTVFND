<?php
    ...
    $nbTotalFinders = Finder::getNombreFinder($connexion);
    $nbFinderParPage = 15;
    $nbPageTotal = ceil($nbTotalFinders/$nbFinderParPage);

    //recuperation de la liste des finder
    $texte = "SELECT * FROM finder LIMIT :debut, :fin";
    $request = $connexion->prepare($texte);
    $request->bindValue(':debut', intval(($page-1)*$nbFinderParPage), PDO::PARAM_INT);
    $request->bindValue(':fin', intval($nbFinderParPage), PDO::PARAM_INT);
    $request->execute();
...
<html lang="en">
        ...
        <div role="navigation" class="navbar navbar-inverse navbar-fixed-top">
            ...
            <div id="navbarCollapse" class="collapse navbar-collapse">
                <ul class="nav navbar-nav">
                    ...
                    <li class="active"><a href="finder_liste.php"><i class="fa fa-search-plus fa-fw"></i>Finder <span class="badge">
    ...
    <div class="container">
        <header>
            <h3 style="display: inline-block">Liste des finders |</h3> <a href="#">Tout afficher</a>
            <form class="pull-right" style="margin-top: 20px;margin-bottom: 10px;" method="POST">
                ...
                <input type="hidden" name="frompage" value="finders">
            </form>
            <hr style="clear: both">
        </header>
        <div class="txt-intro row">
            <?php while($donnees = $request->fetch(PDO::FETCH_ASSOC)){ ?>
            <div class="col-lg-2" id="sys-info-block">
                <div style="">
                    <div><label>Nom : </label><span><?=$donnees['nom_finder']?></span></div>
                    <div><label>Prénom : </label><span><?=$donnees['prenom_finder']?></span></div>
                    <div><label>Tél : </label><span><?=$donnees['tel_finder']?></span></div>
                </div>
                <div>
...
<!--            <div class="sys_amdin-pagination">
        ...
        <div class="row home-middle-content">
            <div class="col-lg-12" align="center">
                <?=ctrl_pagine($nbTotalFinders, $page, $nbFinderParPage, '?p=')?>
            </div>
        </div>
        <div class="">

<?php
    ...
    $nbTotalObjets = Objet::getNombreObjets($connexion);
    $nbObjetParPage = 15;
    $nbPageTotal = ceil($nbTotalObjets/$nbObjetParPage);

    //recuperation de la liste des finder
    $texte = "SELECT * FROM objet LIMIT :debut, :fin";
    $request = $connexion->prepare($texte);
    $request->bindValue(':debut', intval(($page-1)*$nbObjetParPage), PDO::PARAM_INT);
    $request->bindValue(':fin', intval($nbObjetParPage), PDO::PARAM_INT);
    $request->execute();

    //on recupere le nombre de finder trouvés dans la bd
    $nbObjet = $request->rowCount();
?>
<!Doctype html>
<html lang="en">
    <head>
    ...
    <body>
            ...
            <div role="navigation" class="navbar navbar-inverse navbar-fixed-top">
                ...
                <div id="navbarCollapse" class="collapse navbar-collapse">
                    <ul class="nav navbar-nav">
                        ...
                        <li class="active"><a href="objets_liste.php"><i class="fa fa-cube fa-fw"></i>Objets <span class="badge">
        ...
        <div class="container">
            <header>
                <h3 style="display: inline-block">Liste des Objets |</h3> <a href="#">Tout afficher</a>
                <form class="pull-right" style="margin-top: 20px;margin-bottom: 10px;" method="POST">
                    ...
                    <input type="hidden" name="frompage" value="objets">
                </form>
                <hr style="clear: both">
            </header>
            <div class="txt-intro">
                <?php while($donnees = $request->fetch(PDO::FETCH_ASSOC)){ ?>
                    <div class="col-lg-2" id="sys-info-block">
                        <div style="">
                            ...
                            <div><label>Adresse : </label><span><?=$donnees['addr_objet']?></span></div>
                            <div><label>Détails : </label><span><?=$donnees['details_objet']?></span></div>
                        </div>
                        <div>
            ...
            <div class="row home-middle-content">
                <div class="col-lg-12" align="center">
                    <?=ctrl_pagine($nbTotalObjets, $page, $nbObjetParPage, '?p=')?>
                </div>
            </div>
        </div>
    </body>

<?php
    ...
    $nbTotalExps = Cas::getNombreCasExpires($connexion);
    $nbExpsParPage = 15;
    $nbPageTotal = ceil($nbTotalExps/$nbExpsParPage);

    if(isset($_POST['q'])&&!empty($_POST['q'])){
        //purgation des variables avant envoi
        if(true)
            redirecWithParams('adm-search','q='.$_POST['q'].'&frompage='.$_POST['frompage']);
    }
    ...
    //recuperation de la liste des exps
    $texte = "SELECT * FROM cas, finder, objet WHERE cas.id_finder=finder.id_finder AND cas.id_objet=objet.id_objet AND ";
    $texte.= "cas.statut_cas=1 ORDER BY cas.datenaiss_cas DESC LIMIT :debut, :fin";
    $request = $connexion->prepare($texte);
    $request->bindValue(':debut', intval(($page-1)*$nbExpsParPage), PDO::PARAM_INT);
    $request->bindValue(':fin', intval($nbExpsParPage), PDO::PARAM_INT);
    $request->execute();
...
<html lang="en">
    ...
    <body>
        ...
        <div class="container">
            <header>
                <h3 style="display: inline-block">Liste des cas expires<i class="fa fa-trash-o fa-fw"></i> |</h3> <a href="#">Tout afficher</a>
                <form class="pull-right" style="margin-top: 20px;margin-bottom: 10px;" method="POST">
                    ...
                    <input type="hidden" name="frompage" value="exps">
                </form>
                <hr style="clear: both">
            </header>
            <div class="txt-intro">
                <?php while($donnees = $request->fetch(PDO::FETCH_ASSOC)){ ?>
                    <div class="col-lg-2" id="sys-info-block">
                        <div style="">
                            ...
                            <div><label>Statut : </label><span class="red-color"><?php if($donnees['statut_cas']==0) echo 'En cours'; else echo 'Expiré';?></span></div>
                        </div>
                        <div>
                            <footer class="pull-left">
                                <label class="dropdown">
                                    ...
                                    <ul class="dropdown-menu">
                                        ...
                                        <li><a href="cas_modif_file.php?action=modif&status=ok&cas_hash=<?=$donnees['id_cas']?>&from=expires"><i class="fa fa-edit fa-fw orange-color"></i>Modifier</a></li>
...
<!--                <div class="sys_amdin-pagination" align="center">
            ...
            <div class="row home-middle-content">
                <div class="col-lg-12" align="center">
                    <?=ctrl_pagine($nbTotalExps, $page, $nbExpsParPage, '?p=')?>
                </div>
            </div>
            <div class="">
